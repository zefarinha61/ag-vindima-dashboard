...
export default function GrauKgAnalytics({ data }: GrauKgAnalyticsProps) {
    ...
    // 3. Data for Line Chart (Evolução Temporal do Grau/Kg)
    const timeData = useMemo(() => {
        ...
        return Array.from(map.entries())
            .map(([date, valor]) => ({ date, valor: Math.round(valor) }))
    ...
    const topSociosData = useMemo(() => {
        ...
        return Array.from(map.entries())
            .map(([, info]) => ({ name: info.nome.slice(0, 35) + (info.nome.length > 35 ? '...' : ''), valor: Math.round(info.valor) }))
    ...
    const CustomTooltip = ({ active, payload, label, unit = 'Kilograus' }: any) => {
